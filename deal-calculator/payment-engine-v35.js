// Sunbot Deal Calculator V35 — single authoritative payment engine, 19/09/2026
(function(){
'use strict';
...
function ensureStructure(){
  // V35 owns the month selector and the payment schedule structure. No legacy
  // payment overlay is required or allowed to create/update these nodes.
  const months=$('months');
  const monthsBox=months?.closest('.control');
  if(monthsBox&&!$('startMonthControl')){
    const d=document.createElement('div');
    d.id='startMonthControl';d.className='control';
    d.innerHTML='<div class="head"><label>Tháng bắt đầu triển khai</label><span class="value" id="startMonthVal">Tháng 9</span></div><select class="input" id="startMonth"><option value="9">Tháng 9 — đầu năm học</option><option value="10">Tháng 10</option><option value="11">Tháng 11</option><option value="12">Tháng 12</option><option value="1">Tháng 1 — học kỳ II</option><option value="2">Tháng 2</option><option value="3">Tháng 3</option><option value="4">Tháng 4</option><option value="5">Tháng 5</option></select><div class="sub">Tự tính số tháng còn lại đến hết tháng 5.</div>';
    monthsBox.before(d);
